...
function reemplazarColumnas(matrizB,posicionMinimoO,matrizAnum,posicionMinimoR,vectorXb,vectorCbnum,vectorX,vectorCnum){
  for (let i = 0; i < matrizB.length; i++) {
    ...
    var texr =entrasale.textContent = "Ingresa la variable "+vectorX[posicionMinimoR]+" y sale "+vectorXb[posicionMinimoO]
...
function solucionesAlternativas(vectorX, vectorXb, resultado3) {
  ...
  if (posicionesRestantes.some(posicion => resultado3array[posicion] === 0)) {
    ...
    solucionesalternativas.textContent = "Hay múltiples soluciones óptimas. Existen varias soluciones que satisfacen los criterios de optimización"
  }
}
...
function incompatible(vectorXb, resultado3) {
  ...
  if (posicionesM.length > 0) {
    ...
    incompatible.textContent = "Problema es incompatible y no tiene solución factible ya que se llega a una tabla aparentemente óptima y aun quedan variables aritificales en la base"
    error=true;
